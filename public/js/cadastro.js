async function cadastrarUsuario() {
  ...
  if (verificacao.includes("")) {
    ...
    let mensagem = 'Nome ou Sobrenome invalido! Minimo 3 caracteres';
    return mostrarErro(mensagem);

  } else if (email.indexOf("@") < 0 || email.indexOf(".com") < 0) {
    let mensagem = "Email inválido! O email precisa conter '@' e '.com' ex: seunome@provedor.com";
    return mostrarErro(mensagem);
  ...
  try {
    const resposta = await fetch("/usuarios/register", {
      ...
      body: JSON.stringify({
        output_tipoCadastro: tipoCadastro,
        output_nome: nome,
        output_sobrenome: sobrenome,
        output_email: email,
        output_senha: senha,
      }),
    });
